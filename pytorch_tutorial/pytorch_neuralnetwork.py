# -*- coding: utf-8 -*-
"""pytorch_NeuralNetwork.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1qPjzLBMV4XHIEASfGafDD047JsMB0hVo
...
print(f'Precdicted class : {y_pred}')
...
백터는 길이를 변경해주고, 스칼라는 차원을 변경해준다 

NN모델에 정형데이터에서 뽑은 피처를 임베딩으로 넣어주어야 할 일이 있다.

피처가 1개라면 그대로 nn.Linear로 뽑아주면 임베딩 공간에 그대로 벡터표현이 되겠지만..

여러개일 경우 한번에 공간에 흩뿌리듯 펼치면 모델이 학습하기 어려울 것이라 판단.

임베딩은 여기서 Neural Network에 데이터를 의미있게 넣어준다는 느낌?
encoding 방식이랑 비슷하다고 생각?
PCA랑 비슷하다고 생각?
